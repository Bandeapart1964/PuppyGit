...
//    var version = SettingsVersion.commonStartVer  // since version 1
    ...
    var editCacheEnable:Boolean = true,  //是否启用编辑缓存，修改后重启app生效
    ...
    var enableFileSnapshot:Boolean = true,  //是否允许创建文件快照，重启app生效
    var enableContentSnapshot:Boolean = true,  //是否允许创建内容快照，重启app生效。（ps 内容就是编辑文件时在内存中但还未写入到硬盘的内容）
)
