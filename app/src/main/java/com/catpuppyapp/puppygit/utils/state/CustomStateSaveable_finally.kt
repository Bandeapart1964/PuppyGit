...
//class CustomStateSaveable_finally<T : Any>(
...
//        get() {
////            if(debugModeOn) {
////                println("get(): keyPrefix=${keyPrefix}, state.value=${state.value}")
////            }
//
////            return field  //这个获取的值，有误！
//            return cacheStorage.getByType(genKey(keyPrefix, state.value)) ?: field
////            return cacheStorage.get(genKey(keyPrefix, cacheStorage.get(keyPrefix) as String)) as T
//        }
//        set(value) {
//
//            //更新field
...
//
//                //updateKey的作用只是刷新，数据没变，只是换个key，触发页面重新获取最新的数据(页面有时候不会自动刷新，如果状态没变，这个updateKey就是为了解决那种情况下的画面信息有误)
////                cacheStorage.updateKey(oldKey, newKey, requireDelOldKey = false)
//
//                //执行到这，newKey就已经取代oldKey在cache中的作用了，后续更新和恢复都使用newKey，所以在这把最新的state值存上
////                cacheStorage.set(keyPrefix, newStateVal)
//
////                if(debugModeOn) {
...
//
//                //设置新值
////                if(requireSetNewValue) {  //如果恢复出错，把这里改成不删旧值，因为恢复的时候有可能不是新值，而是新值的上一个值，不过20240417我改用map替代rememberSaveable了，所以恢复出错的可能性不大
////                    cacheStorage.del(oldKey)
//                cacheStorage.set(newKey, value)  //更新缓存
//                //如果有bug，可把oldKey也设置成最新的value，这样会变成rememberSaveable管理的状态的最新和前一个状态都关联最新的数据，这样数据有可能和rememberSaveable恢复的数据不一致，但我测试过，好像问题不大
////                    cacheStorage.set(oldKey,value)  //如果恢复出错，remember的值是旧的，可取消这行注释替换删除或set null
//
//                //不知道为什么，set oldKey为null就会导致空指针异常
////                    cacheStorage.set(oldKey, null)  //用设置为null替代删除，减少map重组，不过好像，hashmap实现是取模定bucket，然后里面是链表？那样的话其实删除性能开销也不大，算了，保险起见，还是用设为null吧
////                }
//
//                //修改state变量触发页面刷新
...
//
////                if(debugModeOn) {
////                    println("old key in set="+oldKey)
////                    println("new key in set="+newKey)
...
//        val oldV = cacheStorage.getByType<T>(restoreOrInitKey)
////        if(debugModeOn) {
////            println("restore state.value="+state.value)
////            println("restore key="+restoreOrInitKey)
////            println("oldV="+oldV)
////        }
//        //旧值为null则用初始值，否则用旧值，旧值即恢复的值
//        val restoreOrInitValue = oldV ?: initValue
...
//        //清的时候一定要加keySeparator，不然会把keyPrefix关联的state value也清掉，那就无法恢复数据了
////        cacheStorage.clearByKeyPrefix(keyPrefix+Cache.keySeparator)  //清空之前存储的相同key前缀的值，如果有的话，这个操作一定要放在set前面，不然新key也会被清掉
//        cacheStorage.set(restoreOrInitKey, restoreOrInitValue)  //清完key重新设置value
//        value = restoreOrInitValue
//        isInit = false
//
////        requireRefreshView()
//
...
//
//        other as CustomStateSaveable_finally<*>
//
...
// */
//@Composable
//fun <T : Any> mutableCustomStateOf_finally(keyPrefix: String, initValue: T): CustomStateSaveable_finally<T> {
////    val restoreValueForKeyPrefix = cacheStorage.get(keyPrefix) as? String
////    val willSaveKeyPrefixValue = restoreValueForKeyPrefix?: getShortUUID()
//    val state = rememberSaveable { mutableStateOf(getShortUUID()) }
////    cacheStorage.set(keyPrefix, willSaveKeyPrefixValue)
//    return CustomStateSaveable_finally(keyPrefix, initValue, state)
//}
